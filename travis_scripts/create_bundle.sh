...
# 2. Create staging and production bundles *without* decripted certificates.
...
zip $TRAVIS_BUILD_DIR/$BUNDLE_PATH/$STAGING_BUNDLE_NAME $CERT_PATH/*-staging.pem
# add production keys to the production archive
zip $TRAVIS_BUILD_DIR/$BUNDLE_PATH/$PRODUCTION_BUNDLE_NAME $CERT_PATH/*-production.pem
